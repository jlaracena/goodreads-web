...
App web personal para visualizar y priorizar mi lista de lectura de Goodreads, con una calculadora de plan anual de libros.

Corre localmente en el Mac mini y es accesible desde cualquier dispositivo en la red.
...
| `/own-paper/` | Shelf `own-paper` (libros físicos que tengo) |

La tabla es ordenable por cualquier columna con click.
...
GOODREADS_USER_ID=tu-user-id   # ej: 7069563-juan
```
...
Luego abrir `http://localhost:8766` (o `http://nombre-del-mac.local:8766` desde otro dispositivo en la red).

---

## Correr al inicio del sistema (macOS)

El repo incluye un `com.jinshi.goodreads.plist` de ejemplo para `launchd`. Ajusta las rutas según tu usuario y luego:
